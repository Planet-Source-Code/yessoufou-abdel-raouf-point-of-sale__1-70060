...
Begin VB.Form frm_SALES_PAYMENT_REP 
   ...
   End
End
Attribute VB_Name = "frm_SALES_PAYMENT_REP"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Public lngSalesID As Long
Dim strCompanyName As String

Private Sub sub_GENERATE_RECEIPT(lngID As Long)
On Error GoTo errHandler
    ...
    Set rec = cls_RECEIPTS_Obj.fn_LOAD_RECEIPTS_DETAILS(lngID)
    
    Set myAppReport = myApp.OpenReport(App.Path & "\Reports\CashSales_Rep.rpt")
    Call myAppReport.Database.SetDataSource(rec, 3, 1)
    myAppReport.FormulaFields.GetItemByName("CompanyName").Text = "'" & strCompanyName & "'"
    With CRViewer
        .ReportSource = myAppReport
...
Private Sub Form_Load()
    Call sub_GENERATE_RECEIPT(lngSalesID)
End Sub
